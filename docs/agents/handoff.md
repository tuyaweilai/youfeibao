...
> 实测（租户 1 的 `admin`）：一级菜单共 **18 个**，其中只有 3 个是回收业务（反向开票 / 租户开票就绪 / 平台运营），其余是 yudao 脚手架、已禁用模块的死菜单（报表 / 商城 / CRM / 公众号 / AI / IoT）与三条外链。详见《回收企业 PC 端菜单骨架》。

## 回收企业 PC 端菜单骨架（已定，未实施）

2026-09-20 定。决策见 `docs/adr/0025`–`0028`；评估依据见 `docs/research/2026-09-20-yudao-ERP-契合度评估.md`。**菜单未改，代码未动**，只落了 ADR 与建表 SQL。
...
- 三级菜单进**租户套餐**（`tenant_package.menu_ids`），权限收口到菜单与角色-菜单（ADR 0026）。
- **平台运营不属于租户菜单**：它是系统租户（`packageId = TenantDO.PACKAGE_ID_SYSTEM`），菜单全量。
- 曾经的「租户开票就绪」一级菜单取消：配置项进「基础资料」，**就绪状态**降级为工作台顶部徽标。
- 「ERP 系统」那一棵（`system_menu` id 2563–2702，140 行）不出现：`sale` / `finance` / `account` 与 `statistics` 销售侧不分配菜单，其余收敛进「采购管理 / 仓储管理」。
- 菜单清场（清掉禁用模块的死菜单、三条外链与演示菜单）尚未做。
...
- **frontier（现在就能 grab）**：`#39` 放开 ERP 并让本地能起 / `#40` 权限收口 / `#41` 菜单清场与租户套餐骨架。三张互不阻塞，可并发。
