...
--      都会按套餐把授权**重新加回来**。
--   2. 执行本文件。第 1 段是预览（每个租户将删多少行），第 2 段删除并打印实际删除行数，
--      第 3 段复核（套餐 200 的租户应当剩 0 行）。
--   3. 租户端**退出重新登录**：前端把菜单树缓在 localStorage 的 `roleRouters`，不重登看不到变化。
--
-- 影响面（这是共享表，逐条说清）：
...
WHERE `rm`.`deleted` = 0
  AND `rm`.`menu_id` IN (1078, 1082, 1083, 1084, 1085, 1086, 1088, 1089)
