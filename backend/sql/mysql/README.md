...
22f. `icbc-acquisition-purchase-link.sql` —— 收购单关联采购安排与「直接收购」（#51 T13，ADR 0027）：`icbc_acquisition` 加 `purchase_order_id` / `purchase_order_item_id`（`NOT NULL DEFAULT 0`，0 = 未关联即「直接收购」）。必须在 `icbc-acquisition.sql` 与 `icbc-purchase-order.sql` 之后
