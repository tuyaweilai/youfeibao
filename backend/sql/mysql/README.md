...
22l. `icbc-wallet-columns-drop.sql` —— 一次性清除钱包残留列（#87，ADR 0035，**不可逆**）：`icbc_payee_info` / `icbc_payee_bank_card_change` / `icbc_payer_info` 各删 `icbc_medium_id` 与 `icbc_openacct_status`。工行只回审核结论后这些列已无写入方；建表脚本已是无这些列的最终形状，新库不需跑本文件。必须在 `icbc_payee_info.sql` / `icbc-bank-card-change.sql` / `icbc_payer_info.sql` 之后
