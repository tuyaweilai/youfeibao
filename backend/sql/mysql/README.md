...
22k. `icbc-bank-card-change-account-code.sql` —— 换卡单补「是否我行卡」（#86，ADR 0035）：`icbc_payee_bank_card_change` 加 `account_code`（0-非我行用户 / 1-我行用户）。换卡改走工行**收方修改数据接口**后，这个字段属于换卡发起侧，不再从入驻提交 VO 借。必须在 `icbc-bank-card-change.sql` 之后
22l. `icbc-wallet-columns-drop.sql` —— 一次性清除钱包残留列（#87，ADR 0035，**不可逆**）：`icbc_payee_info` / `icbc_payee_bank_card_change` / `icbc_payer_info` 各删 `icbc_medium_id` 与 `icbc_openacct_status`。工行只回审核结论后这些列已无写入方；建表脚本已是无这些列的最终形状，新库不需跑本文件。**跑法必须带库名**（`mysql -uroot -p <库名> < icbc-wallet-columns-drop.sql`）：脚本的重复跑守卫按 `DATABASE()` 判存在，不带库名会静默不干活（已在文件里加硬失败守卫拦住）。必须在 `icbc_payee_info.sql` / `icbc-bank-card-change.sql` / `icbc_payer_info.sql` 之后
22m. `icbc-payee-account-code.sql` —— 收方档案补「是否我行卡」（#91，建档向导）：`icbc_payee_info` 加 `account_code`（0-非我行用户 / 1-我行用户）。建档向导在银行卡确认页把本人确认过的值存在档案上，收方入驻发起时优先取它，缺省才回退到原来那个没人确认过的常量 1。必须在 `icbc_payee_info.sql` 之后
22n. `icbc-natural-person-id-validity.sql` —— 自然人主体补「证件签发日期 / 证件截止日期」（#91 修票 SP-1）：`icbc_natural_person` 加 `id_sign_date` / `id_validity_period`（均为 `varchar(10)`，`yyyy-MM-dd`，长期 `9999-12-30`）。#81 决策 5 与 CONTEXT 都要求这两个日期由**自然人主体**持有（平台级身份字段，跨企业复用），#91 第一版只落在收方档案上；回填只补主体上空缺、而档案里有值的项，已有值不覆盖。必须在 `icbc-natural-person.sql` 与 `icbc_payee_info.sql` 之后
22o. `icbc-esign.sql` —— 电子签章的平台级配置与租户开通（#92，ADR 0036 / 0037）：`icbc_esign_config`（平台级唯一一份：环境 / 两套 endpoint / 应用标识 / 密钥 / 回调地址与验签 / 签署链接渠道 / 平台模板；密钥只落后端、界面不回显明文；**全局表**，已登记进 `yudao.tenant.ignore-tables`）+ `icbc_esign_tenant`（租户级：子客编号——我们生成、持久化、不可变、不可重复——激活状态、经办人编号、企业印章编号、合同额度）。未开通即降级、不阻断建档（协议落 `PAPER`）
22o2. `icbc-esign-agreement.sql` —— 框架收购协议补「第三方签署任务号」（#95，ADR 0036）：`icbc_framework_agreement` 加 `sign_task_id`（第三方**合同组任务号**；回调只带子客编号与任务号，靠它定位回具体协议；纸质为空）+ `idx_sign_task_id`。必须在 `icbc-seller-onboarding.sql` 与 `icbc-esign.sql` 之后
22p. `icbc-payer-no-width.sql` —— 付方编号加宽 `varchar(20)` → `varchar(64)`（#106 修票，ADR 0039）：`icbc_invoice_order.payer_no` / `icbc_payment_order.payer_no` 装的是工行报文的 `outVendorId`（付方平台外部编号 / 子商户编号），也就是付方档案的 `partnerPayerId`（38 字符），而列只有 20 —— **预下单一旦成功，写订单就会撞 Data too long**（严格模式直接失败，非严格模式静默截断）。单元测试没发现，是因为测试建表里 `icbc_invoice_order` 写的是 64；两处建表与测试建表已一并对齐。幂等
22o3. `icbc-card-recognition.sql` —— 卡证识别的平台级配置（#103，ADR 0037）：`icbc_card_recognition_config`（平台级唯一一份：`provider`——运行期判定 `stub` / `tencent` / 密钥 / 地域 / endpoint / 超时 / 连通性自检结果 `last_check_result` / `last_check_time`；密钥只落后端、界面不回显明文；**全局表**，已登记进 `yudao.tenant.ignore-tables`）。与电子签章**各立一处**（ADR 0037），不进回收企业套餐
22p. `icbc-payee-unique-key.sql` —— 收方档案唯一键补 `tenant_id`（#97，ADR 0017）：`uk_id_card_no` / `uk_mobile` 从 `(id_card_no, deleted)` / `(mobile, deleted)` 改成 `(tenant_id, id_card_no, deleted)` / `(tenant_id, mobile, deleted)`。收方档案是「自然人 × 回收企业」这一层（`CONTEXT.md`）：同一自然人在两家回收企业就该有两份档案。旧键是全局的，第二家企业建档时 INSERT 直接撞键（500），而 Service 层按租户查、看不到别家的行。旧键**严格强于**新键，所以迁移是纯放开、不校验数据。**必须带库名**跑（守卫按 `DATABASE()` 判存在，不带库名会静默不干活，已加硬失败守卫）；新库直接导 `icbc_payee_info.sql`（已是最终形状），不需要跑本文件。必须在 `icbc_payee_info.sql` 之后
...
for f in ruoyi-vue-pro erp erp-stock-goods-config erp-stock-location-batch erp-supplier quartz \
        02-initial-data yudao-module-enterprise-auth-flow member-2024-01-18 icbc_payee_info \
        icbc_payer_info icbc_invoice_tables icbc_payment_order icbc_invoice_download \
        icbc_api_log_callback enterprise icbc-readiness icbc-evidence \
        icbc-public-token icbc-jobs icbc-seller-onboarding icbc-acquisition \
        icbc-invoice-application icbc-payment icbc-invoice-issuance icbc-red-invoice \
        icbc-quota icbc-tax-declaration icbc-billing icbc-natural-person \
        icbc-settlement icbc-station icbc-seller-portal icbc-appointment \
        icbc-seller-notify icbc-handover-batch icbc-purchase-contract icbc-purchase-order \
        icbc-input-invoice icbc-purchase-order-progress icbc-deal-accepted-quantity icbc-acquisition-purchase-link \
        icbc-stock-in icbc-acquisition-acceptance icbc-stock-ops icbc-bank-card-change \
        icbc-bank-card-change-account-code icbc-payee-account-code icbc-wallet-columns-drop icbc-natural-person-id-validity \
        icbc-handover-logistics-link icbc-esign icbc-esign-agreement icbc-card-recognition icbc-menu logistics-vehicle-driver \
        logistics-transport-task logistics-transport-cargo logistics-archive-gate logistics-transport-process \
        logistics-transport-stop logistics-transport-handover logistics-freight logistics-menu; do
         icbc_invoice_download icbc_api_log_callback enterprise icbc-readiness icbc-evidence icbc-public-token icbc-jobs icbc-seller-onboarding icbc-acquisition icbc-invoice-application icbc-payment icbc-invoice-issuance icbc-red-invoice icbc-quota icbc-tax-declaration icbc-billing icbc-natural-person icbc-settlement icbc-station icbc-seller-portal icbc-appointment icbc-seller-notify icbc-handover-batch icbc-purchase-contract icbc-purchase-order icbc-input-invoice icbc-purchase-order-progress icbc-deal-accepted-quantity icbc-acquisition-purchase-link icbc-stock-in icbc-acquisition-acceptance icbc-stock-ops icbc-bank-card-change icbc-bank-card-change-account-code icbc-wallet-columns-drop icbc-handover-logistics-link icbc-esign icbc-esign-agreement icbc-card-recognition icbc-menu logistics-vehicle-driver logistics-transport-task logistics-transport-cargo logistics-archive-gate logistics-transport-process logistics-transport-stop logistics-transport-handover logistics-freight logistics-menu; do
