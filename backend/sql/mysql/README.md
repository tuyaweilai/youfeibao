...
22b. `icbc-handover-batch.sql` —— 交接批次与有效磅次（#50 T12）：`icbc_handover_batch`（一次物理交接一个批次）+ `icbc_weighing`（每次过磅一条原始读数，只有被选定的那一次参与计量）；`icbc_acquisition` 加 `handover_batch_id` / `weighing_id` / `weighing_seq_no`（计量结果引用有效磅次的值与版本）。
