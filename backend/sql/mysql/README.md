...
22m. `icbc-esign.sql` —— 电子签章的平台级配置与租户开通（#92，ADR 0036 / 0037）：`icbc_esign_config`（平台级唯一一份：环境 / 两套 endpoint / 应用标识 / 密钥 / 回调地址与验签 / 签署链接渠道 / 平台模板；密钥只落后端、界面不回显明文；**全局表**，已登记进 `yudao.tenant.ignore-tables`）+ `icbc_esign_tenant`（租户级：子客编号——我们生成、持久化、不可变、不可重复——激活状态、经办人编号、企业印章编号、合同额度）。未开通即降级、不阻断建档（协议落 `PAPER`）
