...
-- 不含平台运营（那是系统租户自己的前台）。
-- #102：基础设施下的「API 日志」整棵排除（访问日志 + 错误日志）——行里是请求体与异常原文，
-- 错误日志还会留下 DB 撞键报文里的 PII 值（见本文件同一票在异常侧的脱敏）；
-- 访问日志虽经 #98 按字段名脱敏，但仍含 name 等未脱敏字段，同属「请求/异常原文」，一并收口。
-- 平台运营在系统租户（packageId=0）菜单全量，不受影响；租户要查只能找平台运营。
...
FROM (
    WITH RECURSIVE `tree` AS (
        ...
        SELECT `m`.`id` FROM `system_menu` `m` INNER JOIN `tree` `t` ON `m`.`parent_id` = `t`.`id`
        WHERE `m`.`deleted` = 0
          -- #102：显式排除「API 日志」（1083）整棵，不让递归带进它的页面与按钮：
          --   1083 API 日志（目录）
          --   1078 访问日志（页面）／1082 日志导出／1088 日志查询
          --   1084 错误日志（页面）／1085 日志处理／1086 日志导出／1089 日志查询
          -- 这里逐个列 id（而非只删 1083），是为了即使将来子节点被改挂到别的父级也仍然排除。
          AND `m`.`id` NOT IN (1078, 1082, 1083, 1084, 1085, 1086, 1088, 1089)
    )
    SELECT `id` FROM `tree`
) AS `pkg_menu`;
