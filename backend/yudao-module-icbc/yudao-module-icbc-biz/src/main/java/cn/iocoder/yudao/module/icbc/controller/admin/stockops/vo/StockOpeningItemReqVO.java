...
/**
 * 期初导入明细 Request VO（#54 T16）。
 */
@Schema(description = "管理后台 - 期初导入明细 Request VO")
@Data
public class StockOpeningItemReqVO {
    ...
    @Schema(description = "期初数量（正数）", requiredMode = Schema.RequiredMode.REQUIRED, example = "5000")
    @NotNull(message = "期初数量不能为空")
    @DecimalMin(value = "0", inclusive = false, message = "期初数量必须大于 0")
