 ...
 *   <li><b>不新造流程</b>：换卡走既有的收方入驻，新卡审核结果通过
...
public interface PayeeBankCardChangeService {
     ...
     *
     * <p>通过 → 新卡生效（搬到收方档案）并留痕为「已生效」；拒绝 / 开户失败 → 变更单记「已拒绝」，
     * <b>原卡继续有效</b>——所以他不会因为一次换卡失败而付不出钱。
     *
     * @param payeeId        收方编号
     * @param openacctStatus 工行开户状态（原样透传）
     * @param result         工行审核结果 pass / reject（原样透传）
     * @param mediumId       工行账户标识（原样透传）
     * @param rejectReason   拒绝原因
     * @return 收敛后的变更单；无在途变更返回 {@code null}
     */
    IcbcPayeeBankCardChangeDO applyOnboardingResult(Long payeeId, String openacctStatus, String result,
