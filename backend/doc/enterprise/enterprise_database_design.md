...
### 3.2 个人实名认证表 (`member_personal_auth`)

存储个人用户的实名认证信息，主要用于企业法人代表的认证。

```sql
CREATE TABLE `member_personal_auth` (
  `id` bigint(20) NOT NULL COMMENT '主键ID (认证记录ID)',
...
### 3.3 企业认证记录表 (`enterprise_auth_record`)

存储企业的认证过程和结果。

```sql
CREATE TABLE `enterprise_auth_record` (
  `id` bigint(20) NOT NULL COMMENT '主键ID (认证记录ID)',
...
### 3.4 企业资质表 (`enterprise_qualification`)
...
### 3.5 企业门店表 (`enterprise_store`)

存储企业的分店或经营点信息。

```sql
CREATE TABLE `enterprise_store` (
  `id` bigint(20) NOT NULL COMMENT '主键ID (门店ID)',
...
### 3.6 用户企业关系表 (`enterprise_user_relation`)

定义系统用户与企业的关联关系及其角色。

```sql
CREATE TABLE `enterprise_user_relation` (
  `id` bigint(20) NOT NULL COMMENT '主键ID',
  `user_id` bigint(20) NOT NULL COMMENT '用户ID (关联 system_users 或 member_users 表)',
  `enterprise_id` bigint(20) NOT NULL COMMENT '企业ID (关联 enterprise_info 表)',
...
### 3.7 企业审核日志表 (`enterprise_audit_log`)

记录对企业信息、认证、资质等关键操作的审核过程。
...
```mermaid
erDiagram
    enterprise_info ||--o{ enterprise_auth_record : "包含认证记录"
    enterprise_info ||--o{ enterprise_qualification : "拥有资质"
    enterprise_info ||--o{ enterprise_store : "拥有门店"
    enterprise_info ||--o{ enterprise_user_relation : "关联用户"
    enterprise_info ||--o{ enterprise_audit_log : "有审核日志"

    member_personal_auth ||--o{ enterprise_auth_record : "被用于法人认证"

    system_users ||--o{ member_personal_auth : "发起个人认证"
    system_users ||--o{ enterprise_user_relation : "关联企业"
    system_users ||--o{ enterprise_auth_record : "提交认证"
    system_users ||--o{ enterprise_audit_log : "执行审核"

    enterprise_store ||--o{ enterprise_store : "上级门店"
    enterprise_store ||--o{ enterprise_user_relation : "关联用户"

    infra_file ||--o{ enterprise_info : "营业执照"
    infra_file ||--o{ member_personal_auth : "身份证照片"
    infra_file ||--o{ enterprise_auth_record : "提交的执照"
    infra_file ||--o{ enterprise_qualification : "资质文件"
```

**注意:**
- `system_users` 和 `infra_file` 表是假设存在的关联模块表，实际名称可能不同。
- 表中 `*_code` 字段通常用于存储行政区划代码等，方便前端进行省市区级联选择。实际应用中可能需要对应的字典表或服务。
- 枚举类字段（如 `enterprise_type`, `status`）的具体值和含义建议通过系统字典模块进行统一管理和维护。 