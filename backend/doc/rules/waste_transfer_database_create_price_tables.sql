...
CREATE TABLE IF NOT EXISTS `waste_recycler_price_config` (
  ...
  `effective_date` date NOT NULL COMMENT '生效日期',
  `expire_date` date DEFAULT NULL COMMENT '失效日期',
  `is_negotiable` bit(1) DEFAULT 0 COMMENT '是否可议价 (0:固定价格, 1:可议价)',
...
CREATE TABLE IF NOT EXISTS `waste_recycler_business_config` (
  ...
  `allow_client_mode_selection` bit(1) DEFAULT 0 COMMENT '是否允许产废企业选择交易模式 (0:否, 1:是)',
  `quotation_timeout_hours` int(11) DEFAULT '24' COMMENT '竞价模式下报价超时时间(小时)',
  `auto_accept_single_quotation` bit(1) DEFAULT 0 COMMENT '单一报价时是否自动接受 (0:否, 1:是)',
  `enable_price_negotiation` bit(1) DEFAULT 0 COMMENT '是否启用价格协商 (0:否, 1:是)',
  `is_enabled` bit(1) NOT NULL DEFAULT 1 COMMENT '配置是否启用 (0:否, 1:是)',
...
-- 为 waste_transfer_order 表添加复合索引
ALTER TABLE waste_transfer_order 
ADD KEY IF NOT EXISTS idx_enterprise_status_time (producing_enterprise_id, business_status, create_time);

ALTER TABLE waste_transfer_order 
ADD KEY IF NOT EXISTS idx_payment_status_time (payment_status, payment_completed_time);

ALTER TABLE waste_transfer_order 
ADD KEY IF NOT EXISTS idx_recycling_status_time (recycling_enterprise_id, business_status, create_time);
