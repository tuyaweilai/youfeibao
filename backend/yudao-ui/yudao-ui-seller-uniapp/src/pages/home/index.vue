<template>
  <view class="page" :class="{ 'page--empty': !naturalPersonId }">
    <!-- 身份未匹配：明确空态，不给假列表 -->
    <view v-if="!naturalPersonId" class="empty">
...
function contactService() {
  const serviceMobile = profile.serviceMobile
  uni.showModal({
    title: '如何关联交易记录',
    content: serviceMobile
      ? `请联系现场收货员核对建档手机号，或拨打 ${serviceMobile} 咨询。`
      : '请联系现场收货员，让对方为你建单，并核对建档手机号是否与当前登录手机号一致。',
    showCancel: false,
    confirmText: '我知道了'
  })
}
...
.page {
  ...
  &--empty {
    ...
    background:
      radial-gradient(circle at 88% 2%, rgba(58, 149, 255, 0.13), transparent 32%),
      linear-gradient(180deg, #f7faff 0%, #f4f7fb 62%, #f7f8fa 100%);
  }
}
