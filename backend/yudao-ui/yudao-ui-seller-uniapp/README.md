...
- `pages/home`：**首页**——问候与待办数、「常用服务」六个入口（待我确认 / 交易记录 / 收款记录 / 发票税费 / 我的预约 / 我的资料）、最近两条待办、找不到入口时的说明。
- `pages/transaction`：**交易**——待我确认 / 我的记录 / 收款记录三个子页签（原首页里的三块内容原样搬到这里）。
...
- `pages/invoice`：**发票与税费**（独立页，不占底部导航）——年度汇总 + 逐票 PDF 下载；开票 / 税费 / 上传三条状态线分别显示。
- `pages/my`：**我的**——我的资料与客服、收款账户尾号与变更银行卡入口（S7）、企业授权列表与自助撤销、退出登录，外加发票税费 / 我的预约两个入口。
- **底部导航**（2026-09-22 用户反馈后改版，见 `docs/adr/0041`）：首页 / 交易 / 我的三项原生 `tabBar`；原来六个横向页签一屏放不下、只能横向滚动，已整排取消。
  - **改 `pages.json` 后要重启 H5 开发服务器**，否则底部导航不出现（uni 把 `__UNI_FEATURE_TABBAR__` 在服务端启动时就算好了）；`uni.scss` 同理，改了不重启会让用到新变量的页面报「连接服务器超时，点击屏幕重试」——那是样式里的 `Undefined variable`，不是网络问题。
...
src/
  ...
  pages/home/         首页（待办概览与常用服务入口）
  pages/transaction/  交易（待确认 / 记录 / 收款三个子页签）
  pages/my/           我的（资料、收款账户、授权、退出）
  pages/invoice/      发票与税费（独立页）
  pages/settlement/   结算确认与异议
  components/         跨页共用组件（如未匹配到主体的空态）
  composables/        跨页共用逻辑（如「去签署」现取链接）
  styles/             登录后主页面共用的版式
  static/tabbar/      底部导航图标（由 scripts/gen-tabbar-icons.py 生成）
  ...
  utils/nav.ts        底部导航跳转与跨页参数（预选子页签走存储、场站编号只放内存）
  store/auth.ts       登录态单例
