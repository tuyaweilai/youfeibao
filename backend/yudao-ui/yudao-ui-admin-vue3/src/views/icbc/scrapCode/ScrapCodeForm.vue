  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    <el-form ref="formRef" :model="formData" :rules="formRules" label-width="130px" v-loading="formLoading">
      <el-form-item label="报废产品名称" prop="name">
        <el-input v-model="formData.name" placeholder="如：废钢铁" />
      </el-form-item>
...
defineOptions({ name: 'IcbcScrapCodeForm' })
...
const formType = ref('')
const formData = ref<ScrapCodeVO>(buildEmpty())
const formRef = ref()

function buildEmpty(): ScrapCodeVO {
  return { id: undefined, name: undefined, mergedCode: undefined, unit: '吨', taxRate: 0.13, status: 0, remark: undefined }
}

const formRules = reactive({
  name: [{ required: true, message: '报废产品名称不能为空', trigger: 'blur' }],
  mergedCode: [{ required: true, message: '税收分类合并编码不能为空', trigger: 'blur' }],
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await ScrapCodeApi.getScrapCode(id)
    } finally {
...
const submitForm = async () => {
  ...
  try {
    if (formType.value === 'create') {
      await ScrapCodeApi.createScrapCode(formData.value)
      message.success(t('common.createSuccess'))
    } else {
      await ScrapCodeApi.updateScrapCode(formData.value)
