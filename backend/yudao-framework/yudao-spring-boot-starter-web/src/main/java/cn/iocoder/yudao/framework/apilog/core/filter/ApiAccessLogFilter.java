...
public class ApiAccessLogFilter extends ApiRequestFilter {
    ...
    // ========== 默认脱敏名单已抽到 ApiLogSanitizer ==========
    // #101：访问日志与异常日志必须用**同一份**名单，所以名单与递归脱敏逻辑搬到
    // {@link ApiLogSanitizer}（framework/web/core/util）。本 Filter 只负责调用，不再自己持有名单。
    ...
    private boolean buildApiAccessLog(ApiAccessLogCreateReqDTO accessLog, HttpServletRequest request, LocalDateTime beginTime,
        ...
        if (!BooleanUtil.isFalse(requestEnable)) { // 默认记录，所以判断 !false
            Map<String, Object> requestParams = MapUtil.<String, Object>builder()
                    .put("query", ApiLogSanitizer.sanitizeMap(queryString, sanitizeKeys))
                    .put("body", ApiLogSanitizer.sanitizeJson(requestBody, sanitizeKeys)).build();
        ...
        if (BooleanUtil.isTrue(responseEnable)) { // 默认不记录，默认强制要求 true
            accessLog.setResponseBody(ApiLogSanitizer.sanitizeJson(result, sanitizeKeys));
        }
        // 持续时间
    ...
    // ========== 请求和响应的脱敏逻辑，见 ApiLogSanitizer ==========
    // 脱敏名单与递归逻辑已抽到 ApiLogSanitizer（#101），访问日志与异常日志共用同一份口径。
